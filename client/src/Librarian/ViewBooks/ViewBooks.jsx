...
export default function ViewBook(){
    ...
    //selectedStudent - student selected to view a book.
    ...
    useEffect(()=>{
        ...
        Librarian_Navlinks[3].style.color='gray';
    },[])
    //getSemester() - used to get a sem values based on year.
    const getSemester=(e)=>{
    ...
    //getSection() - used to get a sections based on dept,year,sem.
    const getSection=(e)=>{
    ...
    //searchStudents() - will give list of students based on dept,year,sem,section.
    ...
    //viewBookSection() - show the viewbook page with selected student details.
    const viewBookSection=(student)=>{
        setSelectedStudent(student)
        document.getElementById('viewBookSection').style.display='none';
        document.getElementById('viewBookPage').style.display='block';
    }
    return(
        <>
        <div id='viewBookSection' className='common_LibrarianBookSection'>
                        ...
                        {students.map((student,index)=>{
                            return(
                                <tr key={index}>
                                    ...
                                    <td>{student.name}</td>
                                    <td><button type='button' className='common_submitButton' onClick={()=>viewBookSection(student)}>View Book</button></td>
                                </tr>
                            )
                        })}
                    </tbody>
        ...
        <div id='viewBookPage' className='view_issue_returnBook'>
           <ViewBookPage student={selectedStudent}/>
        </div>
        </>
    )
}